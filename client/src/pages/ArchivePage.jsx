import { Archive, Heart, LoaderCircle, RefreshCw } from "lucide-react";
...
export default function ArchivePage() {
  ...
  const [favoritesState, setFavoritesState] = useState("loading");
  ...
  async function loadFavorites() {
    ...
    try {
      const body = await apiRequest("/favorites");
          ...
          <section className="mt-14 border-t border-ink/10 pt-8">
            ...
            <div className="mt-5">
              {favoritesState === "loading" ? (
                <ArchiveStatus label={t("archive.loadingFavorites")} compact />
              ) : favoritesState === "error" ? (
                <ArchiveError label={t("archive.favoritesLoadFailed")} retry={loadFavorites} retryLabel={t("common.retry")} compact />
              ) : (
