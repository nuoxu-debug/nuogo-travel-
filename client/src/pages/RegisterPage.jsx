        ...
        <div className="flex items-center justify-center px-5 py-16 sm:px-10">
          <form onSubmit={submit} noValidate className="w-full max-w-md rounded-lg border border-ink/10 bg-white/78 p-6 shadow-panel backdrop-blur-2xl sm:p-8">
            ...
            <p className="mt-6 text-center text-sm text-ink/60">
              {t("auth.hasAccount")} <Link to={`/login${continuationQuery}`} className="font-bold text-lake">{t("auth.signIn")}</Link>
            </p>
          </form>
        </div>
        <div className="relative hidden min-h-[680px] overflow-hidden lg:block">
          <img src="https://images.unsplash.com/photo-1529921879218-f99546d03a9d?auto=format&fit=crop&w=1600&q=88" alt={t("auth.registerImageAlt")} className="absolute inset-0 h-full w-full object-cover" />
          <div className="absolute inset-x-0 bottom-0 bg-ink/90 p-8 text-white">
            <p className="text-xs font-extrabold uppercase text-gold">{t("auth.registerJourneyLabel")}</p>
            <p className="mt-3 max-w-xl font-display text-3xl font-bold">{t("auth.registerJourneyBody")}</p>
          </div>
        </div>
      </section>
    </AppShell>
