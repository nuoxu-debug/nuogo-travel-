...
const budgetLabels = {
  outboundTransport: ["Outbound transport", "去程交通"],
  returnTransport: ["Return transport", "返程交通"],
...
function pointLabel(point, language = "en") {
  const type = point?.locationType;
  if (language === "zh") return { ORIGIN: "出发地", HOTEL: "酒店", DESTINATION: "目的地" }[type] ?? "地点";
  return type?.replaceAll("_", " ").toLowerCase().replace(/^./, (letter) => letter.toUpperCase()) ?? "Point";
}
...
function ActivityDetails({ activity, onClose, language }) {
    ...
    <div className="fixed inset-0 z-[1200] grid place-items-center bg-ink/45 p-4" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-label={`${poi.name}${language === "zh" ? "详情" : " details"}`} className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-paper p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <dl className="mt-6 grid gap-x-6 gap-y-4 border-t border-ink/10 pt-5 text-sm sm:grid-cols-2">
          <div><dt className="text-ink/45">{language === "zh" ? "标准地点编号" : "Canonical POI"}</dt><dd className="mt-1 font-semibold">{poi.canonicalPoiId}</dd></div>
          <div><dt className="text-ink/45">{language === "zh" ? "主要来源" : "Primary source"}</dt><dd className="mt-1 font-semibold">{poi.primarySource}</dd></div>
          <div><dt className="text-ink/45">{language === "zh" ? "类别" : "Category"}</dt><dd className="mt-1 font-semibold">{poi.category}</dd></div>
          ...
          <div><dt className="text-ink/45">{language === "zh" ? "计划时长" : "Scheduled duration"}</dt><dd className="mt-1 font-semibold">{activity.plannedDurationMinutes} {language === "zh" ? "分钟" : "min"}</dd></div>
          <div><dt className="text-ink/45">{language === "zh" ? "系统估算" : "System estimate"}</dt><dd className="mt-1 font-semibold">{activityCostLabel(activity, language) ?? (language === "zh" ? "没有单独的门票估算" : "No separate entry estimate")}</dd></div>
        </dl>
        <div className="mt-5 rounded-lg bg-ink/[0.035] p-4 text-xs text-ink/55">
          {(poi.sourceRecords ?? []).map((record) => <p key={`${record.provider}-${record.sourceId}`}>{record.provider} {language === "zh" ? "来源记录" : "source record"} {record.sourceId}</p>)}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <section role="region" aria-label={language === "zh" ? "系统计算的行程预算" : "Deterministic trip budget"} className="border border-ink/10 bg-paper p-5">
      ...
      <div className="mt-2 flex items-end justify-between gap-3">
        ...
        <span className="pb-1 text-xs text-ink/45">{language === "zh" ? `总预算 ${cny(summary.budgetFen)}` : `of ${cny(summary.budgetFen)}`}</span>
      </div>
      ...
      <dl className="mt-5 grid gap-2.5">
        {Object.entries(budgetLabels).map(([key, label]) => (
          <div key={key} className="flex items-center justify-between gap-3 border-b border-ink/8 pb-2 text-sm">
            <dt className="text-ink/55">{label[language === "zh" ? 1 : 0]}</dt><dd className="font-bold">{cny(summary.categoriesFen[key])}</dd>
          </div>
        ))}
      </dl>
      <p className="mt-4 font-bold text-jade">{language === "zh" ? `剩余 ${cny(summary.remainingFen)}` : `${cny(summary.remainingFen)} remaining`}</p>
      <p className="mt-1 text-sm text-ink/55">{language === "zh" ? `每人 ${cny(summary.perPersonFen)}` : `${cny(summary.perPersonFen)} per traveller`}</p>
    </section>
  );
}
...
export default function ObjectiveTripWorkspace({ trip, setTrip, access }) {
  ...
  const mapActivities = useMemo(() => [
    ...
    ...day.activities.filter((activity) => activity.poi?.coordinates).map((activity) => mapActivity(activity, language)),
    ...
    <div className="min-h-screen bg-mist">
      <header className="border-b border-ink/10 bg-paper px-5 py-6 sm:px-8">
        <div className="mx-auto flex max-w-[1520px] flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            <p className="text-xs font-extrabold uppercase text-lake">{zh ? "已验证行程工作区" : "Validated trip workspace"} {"\u00b7"} {variant.itinerary.variant}</p>
          ...
          {access?.canEdit && <div className="flex flex-wrap gap-2">
            ...
            <button type="button" aria-label={zh ? "编辑偏好" : "Edit preferences"} onClick={() => setStatus("INVALIDATED")} className="flex min-h-11 items-center gap-2 rounded-lg border border-ink/15 bg-white px-3 text-sm font-bold"><Route className="h-4 w-4" /> {zh ? "编辑偏好" : "Edit preferences"}</button>
            <button type="button" aria-label={zh ? "重新生成行程" : "Regenerate trip"} disabled={busy} onClick={revalidate} className="flex min-h-11 items-center gap-2 rounded-lg bg-ink px-3 text-sm font-bold text-white disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`} /> {zh ? "重新生成" : "Regenerate"}</button>
        ...
        {status !== "FINAL_VALIDATED" && <div role="status" className="mx-auto mt-4 flex max-w-[1520px] items-center justify-between gap-4 rounded-lg bg-amber-50 px-4 py-3 text-sm font-bold text-amber-900"><span>{status}: {zh ? "偏好已更改，请重新验证后再使用此行程。" : "preferences changed; validate before relying on this itinerary."}</span><button type="button" aria-label={zh ? "重新验证行程" : "Revalidate itinerary"} disabled={busy} onClick={revalidate} className="flex min-h-11 shrink-0 items-center gap-2 rounded-lg bg-ink px-4 text-white disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`} /> {zh ? "重新验证" : "Revalidate itinerary"}</button></div>}
      </header>
      ...
      <main className="mx-auto max-w-[1520px] px-4 py-4">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(340px,.72fr)] xl:grid-cols-[minmax(0,1fr)_minmax(340px,.72fr)_310px]">
          ...
          <div className="min-w-0 lg:col-span-2 xl:col-span-1"><BudgetSummary summary={variant.summary} language={language} /></div>
        </div>
      </main>
      <ActivityDetails activity={detail} onClose={() => setDetail(null)} language={language} />
